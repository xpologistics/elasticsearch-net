...
namespace Nest6
{
	...
	internal class VariableFieldAttribute : Attribute
	{
		...
		public VariableFieldAttribute(string fieldName) => FieldName = fieldName;
		...
		where TReadAs : class, IFieldNameQuery, TInterface, new()
	{
		protected override object DeserializeJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
		{
			...
			var query = ReadAs(jo.CreateReader(), typeof(TReadAs), existingValue, serializer);
			...
			var unknownProperties = jo.Properties()
				.Select(p => p.Name)
				.Except(knownProperties.Where(p => p.HasMemberAttribute).Select(p => p.PropertyName));
		...
		protected override void SerializeJson(JsonWriter writer, object value, IFieldNameQuery castValue, JsonSerializer serializer)
		{
			...
			{
				Reserialize(localWriter, value, serializer);
		...
		private static Tuple<JsonProperty, VariableFieldAttribute> GetOrCreateTypeInfo(Type t)
		{
			if (!VariableFields.CachedTypeInformation.TryGetValue(t, out var info))
			{
				...
				if (info == null)
					throw new Exception(
						$"Can not use {typeof(VariableFieldNameQueryJsonConverter<TReadAs, TInterface>).Name} if TInterface has no property with VariableFieldAttribute set");
